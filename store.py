...
class MessageStore:
    ...
    def _append_protected_batch(self, session_id: str,
        ...
        for i, (msg, est) in enumerate(zip(messages, token_estimates)):
            ...
            # Ensure unique timestamps per row to preserve explicit ordering constraint.
            ts = base_ts + (i * 1e-6)
            params.append((
                session_id,
                normalized_source,
        ...
        with self._write_lock, self._conn:
            ...
            # Since executemany does not return lastrowid sequentially in cur, retrieve it using last_insert_rowid
