...
class MessageStore:
    ...
    def _append_protected_batch(self, session_id: str,
        ...
        for i, (msg, est) in enumerate(zip(messages, token_estimates)):
            ...
            # ⚡ Bolt: Add microsecond offset to preserve uniqueness per row as required by tests
            ts = base_ts + (i * 1e-6)
            observed_at = _normalize_observed_at(msg.get("timestamp"))
            batch_args.append((
                session_id,
        ...
        with self._write_lock, self._conn:
            # ⚡ Bolt: Use executemany for batch inserts to improve insertion performance
            ...
            # ⚡ Bolt: Retrieve last auto-incremented ID and calculate preceding IDs backward
