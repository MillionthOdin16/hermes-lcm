    ...
    def _search_like(self, query: str, session_id: str | None = None,
        ...
        def add_rows(rows: list[sqlite3.Row]) -> None:
            for row in rows:
                ...
                score = sum(
                    min(count_term_matches(lowered_content, l_term, is_lowered=True), 1) if collapse_risky_repeats else count_term_matches(lowered_content, l_term, is_lowered=True)
                    for l_term in lowered_terms
                )
                if score <= 0:
                    continue
