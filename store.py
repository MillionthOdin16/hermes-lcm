    ...
    def _append_protected_batch(self, session_id: str,
        ...
        for i, (msg, est) in enumerate(zip(messages, token_estimates)):
            ...
            batch_data.append((
                ...
                tc_json,
                msg.get("tool_name"),
                time.time() + (i * 1e-6),  # Ensure unique timestamps per row in batch
                est,
                0,
            ))
        ...
        with self._write_lock, self._conn:
            ...
            # Retrieve auto-incremented IDs for batch inserts
            last_id_cur = self._conn.execute("SELECT last_insert_rowid()")
            last_id = last_id_cur.fetchone()[0]
