...
class MessageStore:
    ...
    def _append_protected_batch(self, session_id: str,
        ...
        normalized_conv = _normalize_conversation_id_value(conversation_id)

        for i, (msg, est) in enumerate(zip(messages, token_estimates)):
            tc = msg.get("tool_calls")
            tc_json = json.dumps(tc) if tc else None
            # Enforce strictly unique timestamps to satisfy regression constraints
            ts = base_time + (i * 1e-6)
            batch_params.append((
                session_id,
                normalized_source,
        ...
        with self._write_lock, self._conn:
            ...
            # Retrieve the last auto-incremented ID and calculate preceding ones
            last_id = self._conn.execute("SELECT last_insert_rowid()").fetchone()[0]
            count = len(batch_params)

            # Since rows are inserted in a single transaction with AUTOINCREMENT,
            # they are assigned contiguous IDs.
