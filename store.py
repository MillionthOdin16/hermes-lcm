    ...
    def _append_protected_batch(self, session_id: str,
        ...
        with self._write_lock, self._conn:
            ...
            for i, (msg, est) in enumerate(zip(messages, token_estimates)):
                ...
                # Add a minor offset to generate strictly unique timestamps for each row
                # to prevent regression tests (like test_append_batch_timestamps_are_unique_per_row) from failing
                ts = base_ts + (i * 1e-6)
                params.append((
                    session_id,
                    _normalize_source_value(source),
            ...
            # `executemany` doesn't set lastrowid correctly for all items in Python's sqlite3 driver.
            # We calculate them backwards from the last inserted row.
            last_id = self._conn.execute("SELECT last_insert_rowid()").fetchone()[0]
            ids = list(range(last_id - cur.rowcount + 1, last_id + 1))
