...
class MessageStore:
    ...
    def _append_protected_batch(self, session_id: str,
        ...
        for i, (msg, est) in enumerate(zip(messages, token_estimates)):
            ...
            # Add microsecond offset to preserve uniqueness regression constraint
            ts = base_ts + (i * 1e-6)
            observed_at = _normalize_observed_at(msg.get("timestamp"))
        ...
        with self._write_lock, self._conn:
            ...
            # executemany doesn't set lastrowid correctly for all inserted rows,
            # so we fetch the final ID and compute backwards.
            row = self._conn.execute("SELECT last_insert_rowid()").fetchone()
            last_id = row[0]
            # Since IDs are contiguous and auto-incrementing:
            return list(range(last_id - len(params) + 1, last_id + 1))
