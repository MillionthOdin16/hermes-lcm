...
class MessageStore:
    ...
    def _append_protected_batch(self, session_id: str,
        ...
        # Pre-compute query parameters to execute in a single batch
        for i, (msg, est) in enumerate(zip(messages, token_estimates)):
            tc = msg.get("tool_calls")
            tc_json = json.dumps(tc) if tc else None
            # Enforce strictly unique timestamps by applying a 1 microsecond offset
            ts = base_time + (i * 1e-6)
            observed_at = _normalize_observed_at(msg.get("timestamp"))
        ...
        with self._write_lock, self._conn:
            ...
            # Retrieve auto-incremented IDs: executemany resets cur.lastrowid, but
            # SELECT last_insert_rowid() fetches the ID of the last row inserted in
            # this connection's most recent batch, allowing us to compute IDs backwards.
