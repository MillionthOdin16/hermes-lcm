...
class MessageStore:
    ...
    def _append_protected_batch(self, session_id: str,
        ...
        for i, (msg, est) in enumerate(zip(messages, token_estimates)):
            ...
            # Enforce unique timestamps per row in a batch (offset by 1 microsecond)
            ts = base_ts + (i * 1e-6)
            observed_at = _normalize_observed_at(msg.get("timestamp"))
            params.append((
                session_id,
        ...
        with self._write_lock, self._conn:
            ...
            # Fetch the last inserted ID. Since triggers on AFTER INSERT don't interfere with
            # last_insert_rowid() on the main connection, we can use it directly and walk backwards.
