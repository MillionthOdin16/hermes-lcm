...
class MessageStore:
    ...
    def _append_protected_batch(self, session_id: str,
        ...
        # ⚡ Bolt: Execute batch inserts via executemany to avoid python loop overhead.
        # We compute parameters in a loop and offset timestamps slightly to maintain
        # the regression constraint that each message row receives a strictly unique timestamp.
        params = []
        base_time = time.time()
        ...
        with self._write_lock, self._conn:
            ...
            # ⚡ Bolt: Python's sqlite3 executemany sets cur.lastrowid to None.
            # To get IDs for batch inserts without a RETURNING clause (which needs manual parsing here),
            # we query last_insert_rowid() and compute IDs backward using cur.rowcount.
