...
class MessageStore:
    ...
    def _append_protected_batch(self, session_id: str,
        ...
        for i, (msg, est) in enumerate(zip(messages, token_estimates)):
            ...
            # Maintain strict uniqueness for timestamps inside the batch
            # to preserve regression constraints.
            ts = base_ts + (i * 1e-6)
            params.append((
                session_id,
                _normalize_source_value(source),
        ...
        with self._write_lock, self._conn:
            ...
            # Fetch last_insert_rowid() because executemany leaves cur.lastrowid = None.
            # Batch inserts via executemany allocate sequential row IDs in SQLite.
