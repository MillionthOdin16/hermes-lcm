    ...
    def _search_like(self, query: str, session_id: str | None = None,
        ...
        while True:
            ...
            for row in rows:
                ...
                if source and not self._node_matches_source(node.node_id, source, cache=source_match_cache):
                    continue
                lowered_summary = node.summary.lower()
                score = sum(
                    min(count_exact_term_matches(lowered_summary, lowered_term), 1) if collapse_risky_repeats else count_exact_term_matches(lowered_summary, lowered_term)
