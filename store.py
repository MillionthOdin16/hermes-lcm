...
class MessageStore:
    ...
    def _append_protected_batch(self, session_id: str,
        ...
        for i, (msg, est) in enumerate(zip(messages, token_estimates)):
            ...
            # Adding a minor offset for batch insert to preserve strictly unique timestamps
            ts = base_ts + (i * 1e-6)
            observed_at = _normalize_observed_at(msg.get("timestamp"))
            batch.append((
                session_id,
        ...
        return list(range(last_id - rowcount + 1, last_id + 1)) if rowcount is not None and rowcount > 0 else []
