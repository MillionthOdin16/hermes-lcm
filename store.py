...
class MessageStore:
    ...
    def _append_protected_batch(self, session_id: str,
        ...
        for i, (msg, est) in enumerate(zip(messages, token_estimates)):
            ...
            # Adding an offset to ensure timestamps are strictly unique per row
            ts = base_ts + (i * 1e-6)
            observed_at = _normalize_observed_at(msg.get("timestamp"))
            params.append((
                session_id,
        ...
        with self._write_lock, self._conn:
            ...
            # Fetch the last inserted ID. Since SQLite auto-increments IDs continuously during executemany,
            # we can infer the full list of IDs assigned to this batch.
            cur = self._conn.execute("SELECT last_insert_rowid()")
            last_id = cur.fetchone()[0]

            num_inserted = len(params)
            # The auto-incremented IDs are sequential. So first_id is last_id - num_inserted + 1.
