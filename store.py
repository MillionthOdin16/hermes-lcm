...
class MessageStore:
    ...
    def _append_protected_batch(self, session_id: str,
        ...
        # Replaces sequential INSERTs inside a Python loop with a single C-level loop
        # inside SQLite. Reduces Python loop overhead and repetitive statement parsing.
        params = []
        base_ts = time.time()
        ...
        for i, (msg, est) in enumerate(zip(messages, token_estimates)):
            ...
            # Ensure unique timestamps per row to pass uniqueness constraints
            ts = base_ts + (i * 1e-6)
            params.append((
                session_id,
                norm_source,
        ...
        with self._write_lock, self._conn:
            ...
            # Execute SELECT last_insert_rowid() immediately after to get the last ID,
            # then calculate preceding inserted IDs backward.
            cur = self._conn.execute("SELECT last_insert_rowid()")
            last_id = cur.fetchone()[0]

        start_id = last_id - len(messages) + 1
