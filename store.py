...
class MessageStore:
    ...
    def _append_protected_batch(self, session_id: str,
        ...
        for i, (msg, est) in enumerate(zip(messages, token_estimates)):
            ...
            # Ensure unique timestamps per row in a batch
            ts = base_ts + (i * 1e-6)
            params.append((
                session_id,
                _normalize_source_value(source),
        ...
        with self._write_lock, self._conn:
            ...
            # cur.rowcount gets reset when we do another execute() on the same cursor
            rowcount = cur.rowcount
            cur.execute("SELECT last_insert_rowid()")
            last_id = cur.fetchone()[0]
            # Since auto-increment IDs are contiguous within a transaction, calculate backward
