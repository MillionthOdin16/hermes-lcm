...
class MessageStore:
    ...
    def _append_protected_batch(self, session_id: str,
        ...
        # Pre-compute parameters for executemany optimization
        params = []
        for i, (msg, est) in enumerate(zip(messages, token_estimates)):
            tc = msg.get("tool_calls")
            tc_json = json.dumps(tc) if tc else None
            # Add a small microsecond offset to ensure unique timestamps per row
            # which is required by regression tests
            ts = base_ts + (i * 1e-6)
            observed_at = _normalize_observed_at(msg.get("timestamp"))
        ...
        with self._write_lock, self._conn:
            # ⚡ Bolt Optimization: Use executemany for batch inserts to minimize CPython-SQLite overhead.
            ...
            # Since executemany doesn't set lastrowid correctly in Python's sqlite3,
            # and triggers don't clobber it, we can fetch the last inserted ID and work backwards.
            cur_id = self._conn.execute("SELECT last_insert_rowid()").fetchone()[0]

            rowcount = cur.rowcount if cur.rowcount is not None and cur.rowcount > 0 else len(messages)
            start_id = cur_id - rowcount + 1

            ids = list(range(start_id, cur_id + 1))
