...
class MessageStore:
    ...
    def _append_protected_batch(self, session_id: str,
        ...
        with self._write_lock, self._conn:
            ...
            # Since executemany does not return lastrowid for all rows, and we are not using RETURNING,
            # we query last_insert_rowid() which gets the ID of the *last* inserted row.
            # Assuming auto-increment assigns contiguous IDs for the batch.
            last_id = self._conn.execute("SELECT last_insert_rowid()").fetchone()[0]
            batch_len = len(params)
            first_id = last_id - batch_len + 1
