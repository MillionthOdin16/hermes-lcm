...
class MessageStore:
    ...
    def _append_protected_batch(self, session_id: str,
        ...
        for i, (msg, est) in enumerate(zip(messages, token_estimates)):
            ...
            # Enforce strictly unique timestamps per row for batch insertion
            ts = base_time + (i * 1e-6)
            observed_at = _normalize_observed_at(msg.get("timestamp"))
        ...
        with self._write_lock, self._conn:
            ...
            # Fetch the last inserted ID. Since this is an atomic transaction
            # with AUTOINCREMENT, the preceding IDs are strictly sequential.
