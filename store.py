...
class MessageStore:
    ...
    def _append_protected_batch(self, session_id: str,
        ...
        # ⚡ Bolt: Batch insert messages using executemany for significant performance gain.
        # We apply a microsecond offset to timestamps to ensure each row in a batch gets a
        # strictly unique timestamp (required by test_append_batch_timestamps_are_unique_per_row).
        base_ts = time.time()
        params = []
        ...
        with self._write_lock, self._conn:
            ...
            # ⚡ Bolt: Python's sqlite3 executemany does not set cur.lastrowid.
            # Workaround: fetch last_insert_rowid() and calculate the ID range backwards.
