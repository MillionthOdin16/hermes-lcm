...
class MessageStore:
    ...
    def _append_protected_batch(self, session_id: str,
        ...
        for i, (msg, est) in enumerate(zip(messages, token_estimates)):
            ...
            # Add minor offset to ensure strictly unique timestamps per row
            # which is required by some tests (e.g. date-based queries)
            ts = base_ts + (i * 1e-6)
            observed_at = _normalize_observed_at(msg.get("timestamp"))
        ...
        with self._write_lock, self._conn:
            ...
            # So we fetch the last auto-incremented ID of the main table and work backward.
            cur = self._conn.execute("SELECT last_insert_rowid()")
            row = cur.fetchone()
            last_id = row[0]

            # Calculate IDs backwards based on the number of inserted messages
            num_inserted = len(params)
