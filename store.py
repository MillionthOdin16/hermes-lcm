...
class MessageStore:
    ...
    def _append_protected_batch(self, session_id: str,
        ...
        # ⚡ Bolt Optimization: Prepare parameters outside the lock to minimize lock contention
        params = []
        base_ts = time.time()
        ...
        for i, (msg, est) in enumerate(zip(messages, token_estimates)):
            ...
            # ⚡ Bolt Optimization: Ensure unique timestamp per row as required by tests without calling time.time() in a loop
            ts = base_ts + (i * 1e-6)
        ...
        with self._write_lock, self._conn:
            # ⚡ Bolt Optimization: Use executemany for faster batch insertion
            ...
            # ⚡ Bolt Optimization: Use SELECT last_insert_rowid() because sqlite3.executemany sets cur.lastrowid to None.
