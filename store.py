    ...
    def _append_protected_batch(self, session_id: str,
        ...
        with self._write_lock, self._conn:
            ...
            for i, (msg, est) in enumerate(zip(messages, token_estimates)):
                ...
                # Apply a minor offset to maintain strictly unique timestamps per row
                ts = base_ts + (i * 1e-6)
                observed_at = _normalize_observed_at(msg.get("timestamp"))
                batch_params.append((
                    session_id,
            ...
            # Python's executemany() clears lastrowid. Fetch the last inserted ID,
            # which is correctly returned even with AFTER INSERT triggers.
            last_id_row = self._conn.execute("SELECT last_insert_rowid()").fetchone()
            last_id = last_id_row[0]

            # Calculate the preceding inserted IDs backward
