...
class MessageStore:
    ...
    def _append_protected_batch(self, session_id: str,
        ...
        for i, (msg, est) in enumerate(zip(messages, token_estimates)):
            ...
            # Enforce strictly unique timestamps for each row in the batch
            # by applying a minor offset, per Data Model/Testing constraint
            ts = base_time + (i * 1e-6)
            batch_params.append((
                session_id,
                _normalize_source_value(source),
        ...
        with self._write_lock, self._conn:
            ...
            # Execute SELECT last_insert_rowid() immediately after the batch insert
            # to get the last inserted ID, then calculate the preceding IDs backward.
            rowcount = cur.rowcount
            cur.execute("SELECT last_insert_rowid()")
            last_id = cur.fetchone()[0]
            first_id = last_id - rowcount + 1
