...
class MessageStore:
    ...
    def _append_protected_batch(self, session_id: str,
        ...
        for i, (msg, est) in enumerate(zip(messages, token_estimates)):
            ...
            # Generate sequential timestamps with a microsecond offset to preserve uniqueness
            # Tests require strict uniqueness in timestamps for message sequences.
            ts = base_ts + (i * 1e-6)
            params.append((
                session_id,
                _normalize_source_value(source),
        ...
        with self._write_lock, self._conn:
            # executemany optimizes batch insert performance from O(N) query executions to O(1)
            ...
            # Retrieve auto-incremented ID and calculate preceding IDs backward
